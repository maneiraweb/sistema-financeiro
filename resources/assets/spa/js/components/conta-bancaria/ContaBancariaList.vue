<template>
    ...
    <modal :modal="modal">
        <div slot="content" v-if="contaBancariaDelete">
            <h4>Exclusão!</h4>
            <p><strong>Deseja excluir esta conta bancária?</strong></p>
            <div class="divider"></div>
            <p>Nome: <strong>{{contaBancariaDelete.nome}}</strong></p>
            <p>Agência: <strong>{{contaBancariaDelete.agencia}}</strong></p>
            <p>Conta: <strong>{{contaBancariaDelete.conta}}</strong></p>
            <div class="divider"></div>
        </div>
        <div slot="footer">
    ...
    export default {
        ...
        computed: {
            ...
            search: {
                ...
                set(value) {
                    store.commit('contaBancaria/setFilter', value);
                }
            },
            contaBancariaDelete() {
                return store.state.contaBancaria.contaBancariaDelete;
            }
        },
        created() {
            store.dispatch('contaBancaria/query');
        },
        methods: {
            destroy() {
                store.dispatch('contaBancaria/delete').then((response) => {
                    Materialize.toast('Conta bancária excluída com sucesso!', 4000);
                });
            }, openModalDelete(contaBancaria) {
                store.commit('contaBancaria/setDelete', contaBancaria);
                $('#modal-delete').modal('open');
            },
            sortBy(key) {
                store.dispatch('contaBancaria/queryWithSortBy', key);
            },
            filter() {
                store.dispatch('contaBancaria/queryWithFilter');
            }
        },
        events: {
            'pagination::changed'(page) {
                store.dispatch('contaBancaria/queryWithPagination', page);
            }
        }
    };
